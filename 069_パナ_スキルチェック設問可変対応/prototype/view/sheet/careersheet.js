...
$(function(){
	...
	**/
	...
	$document.on('click', '#transVSHJIN', function(){
		pageSubmit('/servlet/MultiEditJinikServlet', 'RESTORE');
	});
	$document.on('click', '#transVSHUNC', function(){
		pageSubmit('/view/sheet/layout_multi/Multi_Vshunc.jsp', 'INIT');
	});
	...
	**/
	...
	$document.on('click', '.CsDeliv #modal-submit-btn', function(){
		execSheetAction( __CS_MODAL_ACTION_CD, true );
	});
...
function openCsDelivModal( actionCd ) {
	...
	if (!$('#modal-window>.CsDeliv').exists()) {
		$('#modal-window').empty();
		$('#modal-window').append( $('.modal-body-src.CsDeliv-src').html() );
	}
...
function refreshChgBtnState( loginId ) {
	...
	$('#actorlist tr[data-actorcd="act-owner"]').find('.btn_del').hide();
}
