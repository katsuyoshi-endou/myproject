...
<%
	...
	// "CHG_ACT" or "ADD_REF"
	String mode = request.getParameter("VHD012Mode");
%>
<%------------------------------------------------------------------------------------------------------------------------------
	HTML
------------------------------------------------------------------------------------------------------------------------------%>
...
<div class="desk"><div class="container">
	...
	<div class="navbarBtns">
		<div class="leftArea">
			<button type="button" id="btn_back" class="btn btn-back">戻る</button>
		</div>
	...
	<div id="result" class="mod-table">
		<table class="stripe">
			...
			<tbody>
				...
				<tr>
					<td>
						<a data-cmpacd="<%= dto.getCmpaCd() %>" data-stfno="<%= dto.getStfNo() %>" href="#"><%= dto.getPersonName() %></a>
					</td>
					<td><%= dto.getDeptNm() %></td>
					<td><%= dto.getClsCNm() %></td>
				</tr>
...
<input type="hidden" name="VHD012Mode" value="<%= request.getParameter("VHD012Mode") %>">
...
$(function(){
	...
	**/
	$(document).on('click', '#btn_back', function(){
		<% if ("CHG_ACT".equals(mode)) { %>
		pageSubmit('/view/sheet/VHD014_CsActors.jsp', 'RESTORE');
		<% } else { %>
		pageSubmit('/view/sheet/VHD013_CsReferers.jsp', 'RESTORE');
		<% } %>
	});
	...
	$(document).on('click', '#result a', function(event){
		...
		makeRequestParameter( "cmpaCd", $(this).attr('data-cmpaCd') );
		makeRequestParameter( "stfNo", $(this).attr('data-stfNo') );
		pageSubmit('/servlet/CsActorServlet', '<%= mode %>');
	});
});
</script>
<%-- GlobalFooter ------------------------------------------------------------------------------------------------------------%>
