...
REM *** タイトルバー表示 ***
TITLE *** 承認者未承認蓄積通知送信処理（sendMisyouninChikusekiMail.bat） ***

ECHO 承認者未承認蓄積通知送信処理を開始します・・・
...
set BATCH_APP_JAR=%CAREER_HOME%\lib\syoninsyaMisyouninChikuseki-batch.jar
...
REM バッチログの出力ディレクトリ
set OUT_LOG_DIR=%CAREER_HOME%\log\batch\misyouninchikuseki
...
REM 管理者への転送方式設定
set PROPS=-DKanrisyaMailSendType=BCC
set PROPS=%PROPS% -DKanrisyaMailAddress=

REM 検索条件の設定
REM 申込登録判定日数
set PROPS=-DMisyouninChikuseki.tourokuHanteiNissu=4

REM 申込終了判定日数
set PROPS=%PROPS% -DMisyouninChikuseki.syuryoHanteiNissu=4
...
REM "*** 承認者未承認蓄積通知送信処理実行 ***"
%JAVA_COMMAND% -VBJclasspath %CLASSPATH% %PROPS% jp.co.hisas.addon.batch.learning.misyouninchikuseki.main.MisyouninChikuseki %BATCH_SYSTEMPROPERTY% %OUT_LOG_DIR% 2> %OUT_LOG_DIR%\misyouninchikuseki_Console.log

ECHO 承認者未承認蓄積通知送信処理が終了しました・・・
