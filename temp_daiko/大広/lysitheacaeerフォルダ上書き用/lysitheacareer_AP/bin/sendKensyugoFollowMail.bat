@echo off
REM
REM All Rights Reserved. Copyright (C) 2005, Hitachi Systems & Services, Ltd.
REM 
REM プロジェクト名　：
REM    Career@Net
REM
REM プログラム名　：
REM    研修後フォローメール送信（sendKensyugoFollowMail.bat）
REM
REM 機能　：
REM 
REM 履歴　：
REM    日付        バージョン  名前         内容
REM    2005/11/22  01.00       前田洋一     新規作成
REM

REM *** タイトルバー表示 ***
TITLE *** 研修後フォローメール送信（sendKensyugoFollowMail.bat） ***

ECHO 研修後フォローメール送信処理を開始します・・・
...
set BATCH_APP_JAR=%CAREER_HOME%\lib\send-kensyugo-followmail.jar
...
REM バッチログの出力ディレクトリ
set OUT_LOG_DIR=%CAREER_HOME%\log\batch\kensyugo
...
REM パラメータの設定
set PROPS=-DSEND_MAIL_1=%SEND_MAIL_1%
set PROPS=%PROPS% -DSEND_MAIL_2=%SEND_MAIL_2%
...
REM "*** 研修後フォローメール送信処理実行 ***"
%JAVA_COMMAND% -VBJclasspath %CLASSPATH% %PROPS% jp.co.hisas.addon.batch.learning.kensyugo_follow.main.KensyugoFollowMailSender %BATCH_SYSTEMPROPERTY% %OUT_LOG_DIR% 2> %OUT_LOG_DIR%\kensyugo_Console.log

ECHO 研修後フォローメール送信処理が終了しました・・・
