...
REM *** タイトルバー表示 ***
TITLE *** 開講通知送信処理（sendKaikoTuchiMail.bat） ***
...
set BATCH_APP_JAR=%CAREER_HOME%\lib\kaikoTuchi-batch.jar
...
REM バッチログの出力ディレクトリ
set OUT_LOG_DIR=%CAREER_HOME%\log\batch\kaikoTuchi
...
REM 開講通知対象年月設定（システム年月日使用時は要コメントアウト）
rem set PROPS=%PROPS% -DKaikoTuchi.nengetsu=200504
...
REM "*** 開講通知送信処理実行 ***"
%JAVA_COMMAND% -VBJclasspath %CLASSPATH% %PROPS% jp.co.hisas.addon.batch.learning.kaikotuchi.main.KaikoTuchi %BATCH_SYSTEMPROPERTY% %OUT_LOG_DIR% 2> %OUT_LOG_DIR%\kaikoTuchi_Console.log

ECHO 開講通知送信処理が終了しました・・・
