...
REM *** タイトルバー表示 ***
TITLE *** 組織情報[登録/更新/削除]（setsosiki.bat） ***

ECHO 組織情報[登録/更新/削除]処理を開始します・・・
...
set BATCH_APP_JAR=%CAREER_HOME%\lib\sosiki-update.jar
...
REM バッチログの出力ディレクトリ
set OUT_LOG_DIR=%CAREER_HOME%\log\batch\setsosiki
...
REM 引数パラメータ
set INPUT_FILE_PATH=%CAREER_HOME%\setsosiki\sosiki.csv
set LOG_FILE_PATH1=%OUT_LOG_DIR%\sosiki_torikomi.csv
set LOG_FILE_PATH2=%OUT_LOG_DIR%\consistency_error.csv

REM Javaコマンド設定
set JAVA_COMMAND=%COSMINEXUS_HOME%\TPB\bin\vbj

REM "*** 組織情報[登録/更新/削除]処理実行 ***"
%JAVA_COMMAND% -VBJclasspath %CLASSPATH% jp.co.hisas.addon.batch.soshikiupdate.main.BatchMain %INPUT_FILE_PATH% %LOG_FILE_PATH1% %LOG_FILE_PATH2% %BATCH_SYSTEMPROPERTY%

ECHO 組織情報[登録/更新/削除]処理が終了しました・・・
